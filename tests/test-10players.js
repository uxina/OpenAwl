...
async function test10Players() {
  console.log('\n🎮 E2E 测试 - 10人局\n');

  try {
    // 创建房间
    console.log('📋 步骤1: 创建房间...');
    const result = await makeRequest('/api/rooms', 'POST', { playerCount: 10 });
    ...
    console.log(`\n❌ 10人局测试失败: ${error.message}`);
    return false;
  }
}

test10Players().then(success => {
